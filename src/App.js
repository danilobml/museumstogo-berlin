import './App.css';
import List from './Components/List/List'
import InputForm from './Components/InputForm/InputForm'

const museums = ["Chicken Museum", "Stasi Museum", "Neue Nationalgalerie", "The Museum of eternal longing for an existence unobtainable in one lifetime"]

// function based component

function App() {
  return (
    <div className="App">
      <div className="list-container">
        <h1>Museums to Go</h1>
        {/* creates list containing props from museums array*/}
        <InputForm museumList={museumList}/>
        <List museums={museums} />
    </div>
    </div>
  );
}

export default App;


const museumList = ['Abguss-Sammlung Antiker Plastik',
    'Academy of Arts Berlin',
    'AdK on Hanseatenweg',
    'AG Märkische Kleinbahn',
    'Alice - Museum for Children',
    'Allied Museum',
    'Alte Nationalgalerie',
    'Altes Museum',
    'Animal Anatomical Theater of Humboldt University of Berlin',
    'Anna Seghers Memorial Centre',
    'Anne Frank Zentrum',
    'Anti-War Museum',
    'Archenhold Observatory',
    'Bauhaus Archive',
    'Berggruen Museum',
    'Berlin Energy Museum',
    'Berlin Museum of Medical History at the Charité',
    'Berlin Musical Instrument Museum',
    'Berlin National Mint',
    'Berlin Story Bunker',
    'Berlin Story Museum',
    'Berlin Underworlds Museum',
    'Berlin U-Bahn Museum',
    'Berlin Wall Memorial and Documentation Centre',
    'Berlin-Dahlem Botanical Garden and Botanical Museum',
    'Berlin-Hohenschönhausen Memorial',
    'Berliner DDR-Motorradmuseum',
    'Berlinische Galerie',
    'Berlin-Marzahn Windmill (Bockwindmühle Berlin Marzahn)',
    'Bezirksmuseum Marzahn-Hellersdorf',
    'BlackBox Cold War',
    'Braun-Sammlung Ettel, Museum für Design',
    'Brecht Weigel Memorial',
    'Britz Windmill (Britzer Mühle)',
    'Bode Museum',
    'Boros Collection',
    'Bröhan Museum',
    'Brücke Museum',
    'C/O Berlin',
    'Charlottenburg Palace',
    'Checkpoint Charlie Museum',
    'Computerspielemuseum Berlin',
    'Daimler Contemporary Berlin',
    'Dalí - Die Ausstellung am Potsdamer Platz',
    'Das Verborgene Museum',
    'DDR Museum',
    'Designpanoptikum',
    'Deutsche Bank KunstHalle',
    'Deutsches Historisches Museum',
    'Die Mauer-Asisi Panorama Berlin',
    'Domäne Dahlem',
    'East Side Gallery',
    'Ephraim-Palais',
    'Ethnological Museum of Berlin',
    'The Feuerle Collection',
    'FHXB Friedrichshain-Kreuzberg Museum',
    'Fire Brigade Museum Berlin',
    'Forest Museum',
    'Forum Willy Brandt Exhibition',
    'Friedrichsfelde Palace',
    'Friedrichswerder Church',
    'Futurium',
    'Gaslaternen-Freilichtmuseum Berlin',
    'Gemäldegalerie',
    'Georg-Kolbe-Museum',
    'German Museum for the Blind',
    'German Museum of Technology',
    'German-Russian Museum',
    'Glienicke Palace',
    'Gipsformerei',
    'Gothic House',
    'Grünauer Wassersportmuseum',
    'Gründerzeitmuseum at Gutshaus Mahlsdorf',
    'Günter Litfin Memorial',
    'Hamburger Bahnhof: Museum of the Present',
    'Hanf Museum Berlin',
    'Haus am Lützowplatz',
    'Haus am Waldsee',
    'Haus der Kulturen der Welt',
    'Herzberge Boiler House Museum',
    'Hohenschönhausen Castle',
    'Hošek Contemporary',
    'House of the Wannsee Conference',
    'Huguenot Museum in the Französischer Dom',
    'Hunting Depot at Jagdschloss Grunewald',
    'Industriesalon Schöneweide',
    'Jewish Museum Berlin',
    'Jagdschloss Grunewald',
    'Jugend Museum',
    'Käthe Kollwitz Museum',
    'The Kennedys (museum)',
    'Keramik-Museum Berlin',
    'Knoblauchhaus',
    'Köpenick City Hall',
    'Köpenick Museum',
    'Köpenick Palace',
    'KPM World',
    'Kulturforum',
    'Kunstbibliothek (Museum collections)',
    'Kunstgewerbemuseum Berlin',
    'Kunsthaus Dahlem',
    'Kunstraum Kreuzberg/Bethanien',
    'Kupferstichkabinett Berlin',
    'KW Institute for Contemporary Art',
    'Labyrinth Childrens Museum',
    'Liebermann Villa',
    'Lipstick Museum René Koch',
    'Luisa Catucci Gallery',
    'MACHmit! Museum for kids',
    'Marienfelde Refugee Center Museum',
    'Märkisches Museum',
    'Madame Tussauds Berlin',
    'MAGICUM - Berlin Magic Museum',
    'Martin-Gropius-Bau',
    'me Collectors Room Berlin',
    'Memorial to the German Resistance',
    'Memorial to the Murdered Jews of Europe',
    'Mendelssohn-Remise',
    'Menschen Museum',
    'Micky Schubert',
    'Military History Museum at Belin-Gatow',
    'Mitte Museum',
    'Mori Ôgai Memorial Site',
    'Museum Blindenwerkstatt Otto Weidt',
    'Museum Flat WBS 70',
    'Museum in the Kulturbrauerei',
    'Museum in the Old Waterworks',
    'Museum for Architectural Drawing',
    'Museum for Communication Berlin',
    'Museum Lichtenberg',
    'Museum Neukölln',
    'Museum of Architecture of the Technical University of Berlin',
    'Museum of Asian Art',
    'Museum of European Cultures',
    'Museum of Film and Television Berlin',
    'Museum of Letters (Buchstabenmuseum)',
    'Museum of Illusions',
    'Museum of Islamic Art',
    'Museum of Natural History',
    'Museum of Photography',
    'Museum of Things',
    'Museum of Unheard Of Things',
    'Museum Pankow',
    'Museum Reinickendorf',
    'Museumsdorf Düppel',
    'Nazi Forced Labour Documentation Centre',
    'Neue Kirche',
    'Neue Nationalgalerie',
    'Neues Museum',
    'New Synagogue Berlin-Centrum Judaicum',
    'nGbK (New Society for Fine Arts)',
    'Nikolaikirche',
    'Nineties Berlin-Alte Münze',
    'Open-Air Gas Lantern Museum',
    'Papestrasse SA Prison Memorial',
    'Pergamon Museum',
    'Pergamon Museum-Das Panorama',
    'Pfaueninsel Palace',
    'Police History Collection',
    'Program',
    'Puppet Theatre Museum',
    'Ramones Museum',
    'Red Cross Museum',
    'Sammlung Scharf-Gerstenberg',
    'Samurai Art Museum',
    'Schloss Britz',
    'Schöneberg Museum',
    'Schönhausen Palace',
    'Schwules Museum',
    'Science Center Berlin',
    'Spy Museum Berlin',
    'Spandau Citadel',
    'Sportmuseum Berlin',
    'St. Nikolai Spandau Church Museum',
    'Stasi Museum',
    'Stasi Unterlagen Archive',
    'Steglitz Museum',
    'Tempelhof Museum',
    'Topography of Terror',
    'Trabi Museum',
    'Tränenpalast: Palace of Tears',
    'Treptow Museum',
    'Urban Nation-Museum for Contemporary Art',
    'Villa Oppenheim (Charlottenurg-Wilmersdorf Museum)',
    'Villa Thiede',
    'Zehlendorf Museum of Local History',
    'Zille Museum',
    'Zweiradmuseum',
];
